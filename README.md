...
| 记忆系统 | 三层跨会话持久化：持仓状态（硬事实）/ 用户画像（偏好画像）/ 日报摘要 Harness（近期市场与持仓建议轨迹），自动注入每次问答的 SystemMessage |
...
fin_stock_agent/
  ...
  services/                   持仓服务、记忆管理器、日报摘要服务、用户服务
  storage/                    SQLite 数据库、ORM 模型（含 daily_report_digests）、缓存封装
